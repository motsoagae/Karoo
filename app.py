...
from src.core.exporter import export_to_docx, export_to_pdf

tabs = st.tabs(["🎯 ATS-MAX", "⭐ BALANCED (Recommended)", "🎨 CREATIVE"])
variant_keys = ["ats_max", "balanced", "creative"]

for tab, key in zip(tabs, variant_keys):
    with tab:
        content = variants.get(key, "")
        st.text_area(f"{key.replace('_',' ').upper()} CV", content, height=500, key=f"v_{key}")

        if content:
            # TXT download
            st.download_button(
                f"📋 Copy {key.replace('_',' ').upper()} (TXT)",
                content,
                f"cv_{key}.txt",
                width="stretch"
            )
            ...
            # DOCX download
            docx_bytes = export_to_docx({"cv_variants": {key: content}})
            if docx_bytes:
                st.download_button(
                    f"📝 Download {key.replace('_',' ').upper()} (DOCX)",
                    data=docx_bytes,
                    file_name=f"cv_{key}.docx",
                    mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                    width="stretch"
                )

            # PDF download
            pdf_bytes = export_to_pdf({"cv_variants": {key: content}})
            if pdf_bytes:
                st.download_button(
                    f"📄 Download {key.replace('_',' ').upper()} (PDF)",
                    data=pdf_bytes,
                    file_name=f"cv_{key}.pdf",
